...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[9999] flex items-start justify-center pt-32 px-4 font-inter" onClick={onClose}>
      <div 
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-4 bg-white px-6 py-4 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <input
              ...
              className="bg-transparent border-none outline-none text-base w-full font-bold text-slate-700"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              autoFocus
            />
            <kbd className="px-2 py-1 bg-slate-100 rounded text-xs font-bold text-slate-400 border border-slate-200">ESC</kbd>
          </div>
        </div>
              ...
              {filtered.slice(0, 8).map((patient) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-normal text-slate-800 group-hover:text-[#00A3FF]">{patient.name}</p>
                    <p className="text-xs text-slate-400 font-bold">{patient.cpf} • {patient.insurance}</p>
                  </div>
                  {patient.isUrgent && (
