    ...
    <div className="space-y-8 animate-in fade-in duration-500 font-inter">
      ...
      <div className="bg-white/40 backdrop-blur-md rounded-[13px] border border-white/50 shadow-2xl overflow-hidden">
        <div className="p-8 border-b border-black/5 flex flex-col md:flex-row gap-6 items-center justify-between bg-white/50">
          <div className="relative w-full md:w-[500px]">
            ...
            <input 
              type="text" 
              className="w-full pl-16 pr-8 py-5 bg-white border border-black/5 rounded-[13px] outline-none focus:border-[#706fd3]/50 text-base font-medium text-[#40407a] transition-all shadow-sm placeholder:text-slate-400 font-semibold"
                  ...
                  >
                    <td className="px-10 py-8">
                      <div className="flex items-center gap-6">
                        <div className="w-14 h-14 rounded-[13px] bg-[#40407a]/5 text-[#40407a] flex items-center justify-center font-bold text-xl shadow-inner group-hover:bg-[#706fd3] group-hover:text-white transition-all">
                          {l.name.charAt(0)}
                        </div>
                        <div>
                    ...
                    <td className="px-10 py-8 text-right">
                      <button className="p-3 text-slate-400 group-hover:text-[#706fd3] transition-all bg-white border border-black/5 shadow-sm rounded-[13px]">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-6" onClick={() => setIsModalOpen(false)}>
          ...
          <div 
            className="relative bg-white rounded-[13px] shadow-2xl w-full max-w-lg p-12 animate-in zoom-in-95 fade-in duration-300 border border-white/20"
            onClick={(e) => e.stopPropagation()}
          >
            <button onClick={() => setIsModalOpen(false)} className="absolute top-8 right-8 p-3 text-slate-400 hover:text-[#ff7675] transition-colors rounded-[13px] hover:bg-red-50">
              <X size={24} />
            </button>
