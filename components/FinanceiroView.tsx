    ...
    <div className="space-y-10 animate-in fade-in duration-700 font-inter">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-3 opacity-80">Fluxo de Caixa e Assinaturas Solara</p>
        </div>
        <button 
          onClick={togglePrivacyMode}
          className={`px-8 py-4 rounded-2xl font-bold flex items-center gap-3 transition-all uppercase text-[10px] tracking-widest shadow-xl active:scale-95 ${
            privacyMode 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          <div key={i} className="bg-white/40 backdrop-blur-md p-10 rounded-[40px] border border-white/50 shadow-2xl relative overflow-hidden group hover:scale-[1.02] transition-all">
            <div className={`absolute top-10 right-10 p-4 rounded-2xl ${kpi.bg} ${kpi.color} shadow-sm border border-white/20`}>
              <kpi.icon size={26} />
            </div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-6">{kpi.label}</p>
            <h3 className="text-4xl font-bold text-[#40407a] tracking-tighter">{formatValue(kpi.value)}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="bg-white/40 backdrop-blur-md rounded-[40px] border border-white/50 shadow-2xl overflow-hidden">
            <div className="p-10 border-b border-black/5 flex justify-between items-center bg-white/50">
              ...
              <div className="flex bg-slate-100 p-1.5 rounded-xl border border-black/5">
                ...
                <button className="px-4 py-2 text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-[#40407a] transition-colors">Pendentes</button>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-black/5">
                  {faturas.map((f, i) => (
                    <tr key={i} className="hover:bg-white/60 transition-colors group">
                      <td className="px-10 py-8 text-[11px] font-bold text-slate-300">{f.id}</td>
                      <td className="px-10 py-8 text-[13px] font-bold text-[#40407a]">{f.patient}</td>
                      <td className="px-10 py-8 text-[13px] font-black text-[#40407a]">{formatValue(f.value)}</td>
                      <td className="px-10 py-8 text-[11px] font-bold text-slate-400">{f.due}</td>
                      <td className="px-10 py-8">
                         <div className="flex justify-center">
                          <span className={`px-5 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest border-2 ${f.color}`}>
                            {f.status}
                          </span>
                         </div>
                      </td>
        ...
        <div className="lg:col-span-4 space-y-10">
            <div className="bg-[#40407a] rounded-[40px] p-10 border border-white/10 shadow-2xl relative overflow-hidden group">
              ...
              <div className="flex items-end justify-between mb-12">
                 ...
                 <div className="text-right">
                    ...
                    <span className="text-[10px] text-white/40 block mt-2">/ mês</span>
                 </div>
              </div>

              <div className="space-y-5 mb-12 border-t border-white/5 pt-10">
                 <div className="flex justify-between items-center text-[10px] font-bold">
                    <span className="text-white/40 uppercase tracking-widest">Especialistas cadastrados</span>
                    <span className="text-white">2 de 4</span>
                 </div>
                 <div className="flex justify-between items-center text-[10px] font-bold">
                    <span className="text-white/40 uppercase tracking-widest">Duração do ciclo</span>
                    <span className="text-[#7ed6df]">Renovação Automática</span>
                 </div>
              </div>

              <button className="w-full bg-[#706fd3] text-white py-6 rounded-[24px] text-[10px] font-black uppercase tracking-[0.3em] hover:scale-[1.02] transition-all flex items-center justify-center gap-4 active:scale-[0.98] shadow-2xl shadow-[#706fd3]/20">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/60 backdrop-blur-md rounded-[32px] p-8 border border-white shadow-xl flex items-center justify-between group hover:border-[#7ed6df]/50 transition-all cursor-pointer">
                 <div className="flex items-center gap-5">
                   ...
                   <div>
                      ...
                      <p className="text-[9px] font-bold text-slate-300 mt-1 uppercase tracking-widest">Fatura de Abril — Solara</p>
                   </div>
                 </div>
